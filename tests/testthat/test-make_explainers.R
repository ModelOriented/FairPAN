test_that("test-make_explainers", {
  ...
  data("adult")
  processed <- preprocess(adult,"salary","sex","Male","Female",c("race"),sample=0.8,
  ...
  single <- Single_explainer(processed$test_y,model1,"classifier",processed$data_test,processed$data_scaled_test,
                             processed$protected_test,privileged = "Male",batch_size = 5,dev=dev, verbose = FALSE)

  dual <- Dual_explainer(processed$test_y,model1,model2,"classifier","classifier2",processed$data_test,
                         processed$data_scaled_test,protected=processed$protected_test,privileged = "Male",
                           batch_size = 5,dev=dev, verbose = FALSE )

  triple <- Triple_explainer(processed$test_y,model1,model2,model3,"classifier","classifier2",
                             "classifier3",processed$data_test, processed$data_scaled_test,
  ...
  group_metric_matrix[1,] <- c(0.758752,0.022506,0.442064,0.137791,0.285749,0.702997,
                               0.833945,0.81922 ,0.821242,1.307578,0.121692,0.665424)
  ...
  group_metric_matrix[1,] <- c(0.758752,0.022506,0.442064,0.137791,0.285749,0.702997,
                               0.833945,0.81922 ,0.821242,1.307578,0.121692,0.665424)
  group_metric_matrix2[2,] <- c(0.613104,0.026764,0.466572,0.128634,0.301585,0.594817,
                                0.771970,0.816799,0.713992,1.137422,0.112090,0.562148)
  ...
  group_metric_matrix2[2,] <- c(0.613104,0.026764,0.466572,0.128634,0.301585,0.594817,
                                0.771970,0.816799,0.713992,1.137422,0.112090,0.562148)
  group_metric_matrix3[3,] <- c(0.390688,0.029487,0.532692,0.118433,0.288045,0.383949,
                                0.694300,0.843263,0.594714,0.848885,0.096848,0.452466)

  expect_true(single$epsilon==0.8)
  expect_true(single$privileged=="Male")
  expect_true(sum(single$protected==processed$protected_test)==length(processed$protected_test))
  ...
  expect_true(sum(dual$protected==processed$protected_test)==length(processed$protected_test))
  ...
  expect_true(sum(triple$protected==processed$protected_test)==length(processed$protected_test))
